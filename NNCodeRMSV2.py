...
for num_agvs in range(1, 6):
    ...
    all_jobs_data = {nset: jobs(nset) for nset in range(1, 11)}
    all_processing_data = {nset: processing(nset) for nset in range(1, 11)}
    num_episodes = 2000
    early_stopping_threshold = -50
    performance_metrics = []

    for layout in range(1, 5):
        for nset in range(1, 11):
            ...
            state_dim = 4 + num_agvs + sum(len(jobs_data.iloc[job, 2:2 + jobs_data.iloc[job, 1]].dropna()) for job in range(num_jobs)) + num_agvs + num_jobs
            num_machines = 5
            action_dim = num_jobs * num_machines * num_agvs

            dqn = DQNScheduler(input_dim=state_dim, output_dim=action_dim).to(device)
            target_dqn = DQNScheduler(input_dim=state_dim, output_dim=action_dim).to(device)
            ...
            for episode in range(num_episodes):
                if episode < int(num_episodes * 0.3):
                    epsilon_decay_rate = 0.999
                elif episode < int(num_episodes * 0.5):
                ...
                while not done:
                    ...
                    if action is None:
                        env.current_time = max(env.current_time, env.get_next_event_time())
                        env.update_resource_states()
                    ...
                    error = abs(reward)
                    replay_buffer.store((state, action, reward, next_state, done), error)
                    train_double_dqn(dqn, target_dqn, replay_buffer, batch_size, gamma, optimizer, beta)
                ...
                beta = min(1.0, beta + (1.0 - beta) / num_episodes)
            ...
            all_performance_metrics.append({
             ...
             # Append the last performance metric to a CSV file
            last_metric = all_performance_metrics[-1]
            last_metric_df = pd.DataFrame([last_metric])
            last_metric_df.to_csv("makespan_results_RMSV2_incremental.csv", mode='a', header=not os.path.exists("makespan_results_RMSV2_incremental.csv"), index=False)
            torch.save(dqn.state_dict(), f'model_layout{layout}_set{nset}_agvs{num_agvs}.pth')

torch.save(dqn.state_dict(), 'ModelRMSPropV2.pth')
all_metrics_df = pd.DataFrame(all_performance_metrics)
all_metrics_df.to_csv("makespan_results_RMSV2.csv", index=False)
...
print("All actions have been saved to actions_logRMS2.json.")